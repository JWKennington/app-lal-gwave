"""GWave Game Application 1: Parameter Estimation Game with only visual
"""
...
def sub_callback_new_event(n_clicks, difficulty, fig_data, fig_history):
    ...
    if n_clicks is not None and n_clicks > 0:
        ...
        # Make the plot
        fig_data = plot.cbc_time_domain(data, scale=True)
...
def update_template(m1, m2, s1z, s2z, difficulty, n_clicks):
    ...
    new_fig_template = plot.cbc_time_domain(data, scale=True)
